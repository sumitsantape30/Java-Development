...
//Its running randomly coz its parallel. and parallel is faster than sequential
	...
	} //for x = 2, for filter it might run in one thread 
	//Output:
	/*		In filter, For x= 3, Thread = main
			In reduce, For x= 0 y= 3, Thread = main
			In filter, For x= 2, Thread = ForkJoinPool.commonPool-worker-3
			In reduce, For x= 0 y= 2, Thread = ForkJoinPool.commonPool-worker-3
			In filter, For x= 1, Thread = ForkJoinPool.commonPool-worker-2
			In reduce, For x= 0 y= 1, Thread = ForkJoinPool.commonPool-worker-2
			In reduce, For x= 1 y= 2, Thread = ForkJoinPool.commonPool-worker-2
			In filter, For x= 4, Thread = ForkJoinPool.commonPool-worker-1
			In reduce, For x= 0 y= 4, Thread = ForkJoinPool.commonPool-worker-1
			In reduce, For x= 3 y= 4, Thread = ForkJoinPool.commonPool-worker-1
			In reduce, For x= 3 y= 7, Thread = ForkJoinPool.commonPool-worker-1
			10 */
	/* its parellel, since multiple threads are running at the same time, 0 is something which will not increase the value it doesnt matter if you do 0+9 or 0+12. 
       - The performace in paraller stream will improve only if the operation is such that you are doing in sequential or there's some independent task to do then only performane will improve otherwise peformance will be same as for loop. Performance does improve in parallel streams.
       - Find first is well suited in sequential. in parallel it will check all the elements. both will give the same answer at the end
       - For FindAny parallel is well suited */
